...
var (
	_ datasource.DataSource              = (*userRoleDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*userRoleDataSource)(nil)
)
...
// NewUserRoleDataSource returns the framework datasource constructor.
...
func (d *userRoleDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schemas.UserRoleDataSourceSchema(ctx)
}
...
func (d *userRoleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg schemas.UserRoleDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &cfg)...)
	if resp.Diagnostics.HasError() {
		return
	}

	httpResp, err := d.api.Gen.GetUserRoleWithResponse(ctx, cfg.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Read user_role datasource", err.Error())
		return
	}
	if httpResp.StatusCode() != http.StatusOK || httpResp.JSON200 == nil {
		resp.Diagnostics.AddError("Read user_role datasource",
