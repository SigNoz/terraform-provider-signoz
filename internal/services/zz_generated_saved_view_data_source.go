...
var (
	_ datasource.DataSource              = (*savedViewDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*savedViewDataSource)(nil)
)
...
// NewSavedViewDataSource returns the framework datasource constructor.
...
func (d *savedViewDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schemas.SavedViewDataSourceSchema(ctx)
}
...
func (d *savedViewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg schemas.SavedViewDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &cfg)...)
	if resp.Diagnostics.HasError() {
		return
	}

	httpResp, err := d.api.Gen.GetSavedViewWithResponse(ctx, cfg.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Read saved_view datasource", err.Error())
		return
	}
	if httpResp.StatusCode() != http.StatusOK || httpResp.JSON200 == nil {
		resp.Diagnostics.AddError("Read saved_view datasource",
