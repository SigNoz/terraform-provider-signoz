...
var (
	_ datasource.DataSource              = (*ruleDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*ruleDataSource)(nil)
)
...
// NewRuleDataSource returns the framework datasource constructor.
...
func (d *ruleDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schemas.RuleDataSourceSchema(ctx)
}
...
func (d *ruleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg schemas.RuleDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &cfg)...)
	if resp.Diagnostics.HasError() {
		return
	}

	httpResp, err := d.api.Gen.GetRuleByIDWithResponse(ctx, cfg.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Read rule datasource", err.Error())
		return
	}
	if httpResp.StatusCode() != http.StatusOK || httpResp.JSON200 == nil {
		resp.Diagnostics.AddError("Read rule datasource",
