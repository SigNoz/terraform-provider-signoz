...
// NewRoleDataSource returns the framework datasource constructor.
...
func (d *roleDataSource) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schemas.RoleDataSourceSchema(ctx)
}
...
func (d *roleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg schemas.RoleDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &cfg)...)
	if resp.Diagnostics.HasError() {
		return
	}

	httpResp, err := d.api.Gen.GetRoleWithResponse(ctx, cfg.Id.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Read role datasource", err.Error())
		return
	}
	if httpResp.StatusCode() != http.StatusOK || httpResp.JSON200 == nil {
		resp.Diagnostics.AddError("Read role datasource",
