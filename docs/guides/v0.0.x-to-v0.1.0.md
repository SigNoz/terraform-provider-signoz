...
~> **Use Terraform 1.9.8 or newer for the config-generation step.** OpenTofu (all releases to date) — and Terraform before 1.9.8 — writes map keys unquoted, and because `spec.panels` is keyed by panel UUIDs the generated HCL fails to parse with `Missing key/value separator` (fixed in Terraform 1.9.8 and 1.10.0; still open upstream as [opentofu/opentofu#3978](https://github.com/opentofu/opentofu/issues/3978)). The generated config is parsed in memory before it is written, so **no config file is written at all** — there is nothing to salvage or hand-fix. If you must stay on OpenTofu, add an empty `resource "signoz_dashboard" "<name>" {}` stub next to the `import` block and run a plain `tofu plan`: the diff shows the full remote object to copy in.
